...
//       applyStatusBarVisibility, importAppearanceSettings, exportAppearanceSettings
  ...
  async function renderCPhoneIconSettings() {
    ...
    for (const iconId of allKeys) {
      ...
      if (rendered < MAX_PER_GRID && url && totalBytes + url.length <= MAX_TOTAL_BYTES) {
    ...
    for (let i = 0; i < visible.length; i += BATCH) {
      ...
      for (const [iconId, iconUrl] of batch) {
        const labelText = cphoneAppLabels[iconId] || '未知App';
        const item = document.createElement('div');
        item.className = 'icon-setting-item';
        item.dataset.iconId = iconId;
        item.innerHTML = `
                    <img class="icon-preview" src="${iconUrl}" alt="${labelText}" loading="lazy" decoding="async" onerror="this.style.opacity='0.3'">
                    <button class="change-icon-btn">更换</button>
                `;
        fragment.appendChild(item);
      }
      grid.appendChild(fragment);
      if (i + BATCH < visible.length) {
